...
(function () {
  ...
  function injectStyle() {
    ...
    style.textContent = `
      ...
      [${MAIN_ATTR}]{
        display:block !important;
        /* Responsive, consistent reading width — never overflows the viewport, never a tiny strip. */
        max-width:min(70ch, 94vw) !important;
        width:auto !important;
        flex:1 1 100% !important;
        grid-column:1 / -1 !important;
      ...
      /* Stack only the TOP-LEVEL columns of the chosen content (fixes the "narrow title"
         squeeze on product pages) — WITHOUT deeply flattening nested layout, so the result
         stays consistent across sites. */
      [${MAIN_ATTR}] > :where(div,section,article,aside,form){display:block !important;float:none !important;width:auto !important;max-width:100% !important;margin-left:0 !important;margin-right:0 !important;}
      /* Anything deeper just must not overflow the column or float. */
      [${MAIN_ATTR}] :where(div,section,aside,figure,img,video,pre,table,iframe){float:none !important;max-width:100% !important;}
